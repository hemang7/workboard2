...
const Column = ({ column, tasks, allData, boardId, userId, filterBy, index, projectId }) => {
    ...
    const deleteCol = (colId, tasks) => {
        ...
        tasks.forEach(t => {
            db.collection(`users/${userId}/projects/${projectId}/boards/${boardId}/tasks`)
                .doc(t)
                .delete()
        })
    }
    ...
    if (!column) {
        return null; // or handle the case where column is undefined
    }
                        ...
                        <Droppable droppableId={column.id} type='task'>
                            {(provided, snapshot) =>
                                <div {...provided.droppableProps} ref={provided.innerRef} className={`shadow-sm h-full py-4 px-2 ${snapshot.isDraggingOver ? 'bg-gradient-to-br from-green-400 via-green-200 to-green-100' : ''}`}>
                                    {tasks.map((t, i) => <Task allData={allData} id={t} index={i} key={t} boardId={boardId} userId={userId} columnDetails={column} filterBy={filterBy} projectId={projectId} />)}
