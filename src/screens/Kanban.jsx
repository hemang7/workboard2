...
import {Add, Github,Home} from '../components/Icons'
...
import {debounce} from '../utils'
    ...
    const Kanban = ({ userId }) => {
        ...
        const { initialData, setInitialData, boardName } = useKanbanData(userId, boardId,projectId);
        const [filter, setFilter] = useState(null);
        const filters = ['high', 'medium', 'low'];
        const [finalData,setFinal] = useState(initialData)
        
        useEffect(()=>{
            setFinal(initialData)
            console.log("UseEffect final Data",finalData);
        },)
	...
	const onDragEnd = (result) => {
        ...
        if(result.type === 'task')  {
            ...
            if(startColumn === endColumn){
                ...
                db.collection(`users/${userId}/projects/${projectId}/boards/${boardId}/columns`).doc(startColumn.id)
            ...
            db.collection(`users/${userId}/projects/${projectId}/boards/${boardId}/columns`).doc(newStart.id)
                .update({taskIds: startTaskIDs})

            db.collection(`users/${userId}/projects/${projectId}/boards/${boardId}/columns`).doc(newFinish.id)
            ...
            db.collection(`users/${userId}/projects/${projectId}/boards/${boardId}/columns`)
                .doc('columnOrder')
    ...
    const changeBoardName = debounce((ev) => {
        db.collection(`users/${userId}/projects/${projectId}/boards`)
            .doc(boardId)
            .update({ name: ev });
    }, 7000);
    ...
    // console.log("project id in kanban : ", projectId)
    // console.log("board id in kanban : ", projectId)

    console.log("initial data in kanban : ", initialData)
    

    // console.log("userId in kanban : ", userId)
    // console.log("PROJECTId in kanban : ", projectId)
    // console.log("boardId in kanban : ", boardId)
            ...
            {initialData ? (
                <>
                    <Modal modal={modal} setModal={setModal} ariaText="Add a new task">
                        <AddTask boardId={boardId} userId={userId} projectId={projectId} allCols={initialData.columnOrder} close={() => setModal(false)} />
                    </Modal>
                            ...
                            <header className="bg-white z-10 text-sm sm:text-base py-5 mx-3 md:mx-6">
                                <div className="flex flex-wrap justify-between items-center">
                                    <span className="text-xl">
                                        <Link to={`/project/${projectId}`} className="text-blue-800 hover:text-blue-500">
                                            Boards{' '}
                                        </Link>
                                        <span className="">/</span>
                                        <input
                                    ...
                                    <div>
                                        ...
                                        <input maxLength="20" className="truncate bg-transparent placeholder-indigo-500 text-indigo-800 bg-indigo-50 px-2 outline-none py-1 rounded-sm ring-2 focus:ring-indigo-500" type="text" name="newCol" placeholder="Enter collaborator email"/> 
                                    ...
                                    <div className="flex flex-wrap items-center sm:space-x-9">
                                        ...
                                        <div className="flex items-center text-blue-700 rounded-sm px-2 py-1 mr-3 hidden sm:flex">
                                            <Home />
                                        <Link to={`/`} className="text-md sm:text-xl ml-1  hover:text-blue-900">Home</Link>
                                        </div>
                                ...
                                <Droppable droppableId="allCols" type="column" direction="horizontal">
                                    {(provided) => (
                                        ...
                                        <div {...provided.droppableProps} ref={provided.innerRef} className="grid overflow-x-auto h-full items-start pt-3 md:pt-2 mx-1 md:mx-6 auto-cols-220 md:auto-cols-270 grid-flow-col" style={{ height: '90%' }}>
                                            ...
                                            {finalData?.columnOrder?.map((col, i) => {
                                                 console.log("Data Kanban-----",col);
                                                const column = initialData?.columns[col];
                                                const tasks = column.taskIds?.map((t) => t);
                                                console.log("Data Kanban-----",col);
                                                return <Column column={column} tasks={tasks} allData={initialData} key={column.id} boardId={boardId} userId={userId} filterBy={filter} index={i} projectId={projectId} />;
                                            })}
